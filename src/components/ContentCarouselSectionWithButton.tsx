...
export interface ContentCarouselSectionWithButtonProps<T> {
  tag?: string;
  title: string;
  description: string;
  buttonText?: string;
...
const ContentCarouselSectionWithButton = <T,>({
  tag,
  title,
  description,
  buttonText,
...
}: ContentCarouselSectionWithButtonProps<T>) => {
    ...
    <section className={`py-20 ${backgroundColor}`}>
      <div className="max-w-5xl mx-auto px-2 md:px-6 lg:px-8 xl:px-8">
        <div className="relative mb-16">
          ...
          >
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-normal text-white">{title}</h2>
            <p className="text-base md:text-[16px] text-white/80 leading-relaxed">{description}</p>
          </div>

          {/* Button and Navigation in one line */}
          <div className="flex items-center justify-between mb-12">
            <div>
              {buttonText && <BorderedButton text={buttonText} onClick={onButtonClick} />}
            </div>
...
export default ContentCarouselSectionWithButton;
